...
public :
	...
	void  Get( int *p_beat_num, float *p_beat_tempo, int *p_beat_clock, int* p_meas_num ) const
	{
	...
	int   get_beat_num   ()const{ return _beat_num   ;}
	float     get_beat_tempo ()const{ return _beat_tempo ;}
	int   get_beat_clock ()const{ return _beat_clock ;}
	int   get_meas_num   ()const{ return _meas_num   ;}
	int   get_repeat_meas()const{ return _repeat_meas;}
	int   get_last_meas  ()const{ return _last_meas  ;}
	...
	void  set_repeat_meas( int meas ){ if( meas < 0 ) meas = 0; _repeat_meas = meas; }
	void  set_last_meas  ( int meas ){ if( meas < 0 ) meas = 0; _last_meas   = meas; }
	void  set_beat_clock ( int beat_clock ){ if( beat_clock < 0 ) beat_clock = 0; _beat_clock = beat_clock; }
	...
	bool io_w_v5( pxtnDescriptor *p_doc, int rough ) const
	{
		...
		if( !p_doc.w_asfile( &bclock      , short.sizeof, 1 ) ) return false;
		if( !p_doc.w_asfile( &bnum        , byte.sizeof, 1 ) ) return false;
		if( !p_doc.w_asfile( &btempo      , float.sizeof, 1 ) ) return false;
		if( !p_doc.w_asfile( &clock_repeat, int.sizeof, 1 ) ) return false;
		if( !p_doc.w_asfile( &clock_last  , int.sizeof, 1 ) ) return false;
	...
	pxtnERR io_r_v5( pxtnDescriptor *p_doc )
	{
		...
		if( !p_doc.r( &size, uint.sizeof, 1 ) ) return pxtnERR.pxtnERR_desc_r;
		if( size != 15                              ) return pxtnERR.pxtnERR_fmt_unknown;

		if( !p_doc.r( &beat_clock  ,short.sizeof, 1 ) ) return pxtnERR.pxtnERR_desc_r;
		if( !p_doc.r( &beat_num    ,byte.sizeof, 1 ) ) return pxtnERR.pxtnERR_desc_r;
		if( !p_doc.r( &beat_tempo  ,float.sizeof, 1 ) ) return pxtnERR.pxtnERR_desc_r;
		if( !p_doc.r( &clock_repeat,int.sizeof, 1 ) ) return pxtnERR.pxtnERR_desc_r;
		if( !p_doc.r( &clock_last  ,int.sizeof, 1 ) ) return pxtnERR.pxtnERR_desc_r;
	...
	int io_r_v5_EventNum( pxtnDescriptor *p_doc )
	{
		uint size;
		if( !p_doc.r( &size, uint.sizeof,  1 ) ) return 0;
		if( size != 15                               ) return 0;
		byte[ 15 ] buf;
		if( !p_doc.r(  buf.ptr , byte.sizeof, 15 )  ) return 0;
		return 5;
	}
	...
	int io_r_x4x_EventNum( pxtnDescriptor *p_doc )
	{
		_x4x_MASTER mast;
		int     size;
		int     work;
