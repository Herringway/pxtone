...
public:
	...
	bool set_file_r  ( FILE *fd )
	{
		...
		fpos_t sz;
		if( fseek  ( cast(REALFILE*)fd, 0, SEEK_END ) ) return false;
		if( fgetpos( cast(REALFILE*)fd, &sz         ) ) return false;
		if( fseek  ( cast(REALFILE*)fd, 0, SEEK_SET ) ) return false;
		_p_desc = fd  ;
	...
	bool seek( pxtnSEEK mode, int val )
	{
		if( _b_file )
		{
			...
			if( fseek( cast(REALFILE*)_p_desc, val, seek_tbl[ mode ] ) ) return false;
		}
		else
		{
			switch( mode )
	...
	bool w_asfile( const(void) *p, int size, int num )
	{
		...
		if( fwrite( p, size, num, cast(REALFILE*)_p_desc ) != num ) goto End;
		_size += size * num;
	...
	bool r(       void *p, int size, int num )
	{
		...
		if( _b_file )
		{
			if( fread( p, size, num, cast(REALFILE*)_p_desc ) != num ) goto End;
		}
		else
		{
			for( int  i = 0; i < num; i++ )
	...
	int  v_w_asfile( int val, int *p_add )
	{
		...
		if( fwrite( b.ptr, 1, bytes, cast(REALFILE*)_p_desc )     != bytes ) return false;
