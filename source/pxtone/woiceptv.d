...
bool _Write_Wave( pxtnDescriptor *p_doc, const(pxtnVOICEUNIT) *p_vc, int *p_total )
{
	...
	case pxtnVOICETYPE.pxtnVOICE_Sampling:
		...
		*p_total += size;
		...
		case pxtnVOICETYPE.pxtnVOICE_OggVorbis: goto End; // not support.
		default: break;
	}
...
pxtnERR _Read_Wave( pxtnDescriptor *p_doc, pxtnVOICEUNIT *p_vc )
{
	...
	// coodinate (3)
	case pxtnVOICETYPE.pxtnVOICE_Coodinate:
		if( !p_doc.v_r( &p_vc.wave.num  ) ) return pxtnERR.pxtnERR_desc_r;
		if( !p_doc.v_r( &p_vc.wave.reso ) ) return pxtnERR.pxtnERR_desc_r;
		num = p_vc.wave.num;
		if( !pxtnMem_zero_alloc( cast(void **)&p_vc.wave.points, pxtnPOINT.sizeof * num ) ) return pxtnERR.pxtnERR_memory;
		for( i = 0; i < num; i++ )
		{
			if( !p_doc.r( &uc, 1, 1 ) ) return pxtnERR.pxtnERR_desc_r; p_vc.wave.points[ i ].x = uc;
			if( !p_doc.r( &sc, 1, 1 ) ) return pxtnERR.pxtnERR_desc_r; p_vc.wave.points[ i ].y = sc;
		}
		num = p_vc.wave.num;
		break;
	// overtone (2)
	case pxtnVOICETYPE.pxtnVOICE_Overtone:
		...
		for( i = 0; i < num; i++ )
		{
			if( !p_doc.v_r( &p_vc.wave.points[ i ].x ) ) return pxtnERR.pxtnERR_desc_r;
			if( !p_doc.v_r( &p_vc.wave.points[ i ].y ) ) return pxtnERR.pxtnERR_desc_r;
		}
		break;

	// p_vc.sampring. (7)
	case pxtnVOICETYPE.pxtnVOICE_Sampling: return pxtnERR.pxtnERR_fmt_unknown; // un-support
	...
	default: return pxtnERR.pxtnERR_ptv_no_supported; // un-support
	}
...
pxtnERR _Read_Envelope( pxtnDescriptor *p_doc, pxtnVOICEUNIT *p_vc )
{
	...
	if( !p_doc.v_r( &p_vc.envelope.head_num ) ){ res = pxtnERR.pxtnERR_desc_r     ; goto term; }
	if( !p_doc.v_r( &p_vc.envelope.body_num ) ){ res = pxtnERR.pxtnERR_desc_r     ; goto term; }
	if( !p_doc.v_r( &p_vc.envelope.tail_num ) ){ res = pxtnERR.pxtnERR_desc_r     ; goto term; }
	if( p_vc.envelope.body_num                 ){ res = pxtnERR.pxtnERR_fmt_unknown; goto term; }
	if( p_vc.envelope.tail_num != 1            ){ res = pxtnERR.pxtnERR_fmt_unknown; goto term; }

	num = p_vc.envelope.head_num + p_vc.envelope.body_num + p_vc.envelope.tail_num;
	if( !pxtnMem_zero_alloc( cast(void **)&p_vc.envelope.points, pxtnPOINT.sizeof * num ) ){ res = pxtnERR.pxtnERR_memory; goto term; }
	for( i = 0; i < num; i++ )
	{
		if( !p_doc.v_r( &p_vc.envelope.points[ i ].x ) ){ res = pxtnERR.pxtnERR_desc_r; goto term; }
		if( !p_doc.v_r( &p_vc.envelope.points[ i ].y ) ){ res = pxtnERR.pxtnERR_desc_r; goto term; }
	}

	res = pxtnERR.pxtnOK;
term:
	if( res != pxtnERR.pxtnOK ) pxtnMem_free( cast(void**)&p_vc.envelope.points );
