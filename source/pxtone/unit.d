...
public :
	...
	void Tone_Velocity ( int val ) nothrow { _v_VELOCITY           = val; }
	void Tone_Volume   ( int val ) nothrow { _v_VOLUME             = val; }
	void Tone_Portament( int val ) nothrow { _portament_sample_num = val; }
	void Tone_GroupNo  ( int val ) nothrow { _v_GROUPNO            = val; }
	void Tone_Tuning   ( float   val ) nothrow { _v_TUNING             = val; }
	...
	const(pxtnWoice) *get_woice() const nothrow { return _p_woice; }
	...
	bool is_name_buf () const nothrow { if( _name_size > 0 ) return true; return false; }
	...
	bool get_operated() const nothrow { return _bOperated; }
	bool get_played  () const nothrow { return _bPlayed  ; }
